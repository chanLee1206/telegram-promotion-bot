...
def fetch_coinData(coinType):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()  
            print("coinData received:", json.dumps(data, indent=4))
        else:
...
def fetch_pair_data(pair_id):
    url = f"https://api.raidenx.io/api/v1/sui/pairs/{pair_id}"    
    try:
        response = requests.get(url)        
        if response.status_code == 200:
            data = response.json()  
            print("Piar data received:", json.dumps(data, indent=4))
        else:
